...
An example sequence of a child's first two years of life, where the mother is first partnered for 12 months (four quarters), then single for 6 (two quarters) and finally repartnered for 6 months (two quarters) is visible in @fig-state-ex.

```{r}
#| echo: false
#| results: "asis"
#| label: fig-state-ex
#| fig-cap: "Two year union state sequence example"
...
## Descriptives
<!-- In @tbl-1 I show basic descriptives of the study population. We see that most children are born between 1980 and 2000, and that the total number is 48,163. We also see that all regions have quite large samples except for Southern Europe, which only have around 4,000 children included from just two countries. This means that we need to take the results from Southern Europe with a grain of salt in the final analysis.  -->
